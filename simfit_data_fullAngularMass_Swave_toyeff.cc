...
void simfit_data_fullAngularMass_SwaveBin(int q2Bin, int parity, bool multiSample, uint nSample, uint q2stat, int fitOption, int XGBv, int unblind, int toy, bool localFiles, bool plot, int save, std::vector<int> years)
{
  ...
  for (unsigned int iy = 0; iy < years.size(); iy++) {
    ...
    string filename = Form((parity==0 ? "KDEeff_b%i_ev_%i_toy%i.root" : "KDEeff_b%i_od_%i_toy%i.root"),q2Bin,years[iy],toy);
    if (!localFiles) {
  ...
  TFile* fout = 0;
  if (save>0) fout = new TFile(("simFitResults4d/simFitResult_data_fullAngularMass_Swave_" + all_years + stat + Form("_b%i%s_toyeff%i.root", q2Bin, XGBstr.c_str(), toy)).c_str(),"RECREATE");
  ...
  for (uint is = firstSample; is <= lastSample; is++) {
    ...
    *params = *savedParams ;
	...
	if (runPostFitSteps) fitter->improveAng(1,30000);
	improvTime.Stop();
	...
	string plotString = shortString + "_" + all_years + stat + XGBstr + Form("_toyeff%i",toy);
...
void simfit_data_fullAngularMass_SwaveBin1(int q2Bin, int parity, bool multiSample, uint nSample, uint q2stat, int fitOption, int XGBv, int unblind, int toy, bool localFiles, bool plot, int save, std::vector<int> years)
{
  if ( parity==-1 )
    for (parity=0; parity<2; ++parity)
      simfit_data_fullAngularMass_SwaveBin(q2Bin, parity, multiSample, nSample, q2stat, fitOption, XGBv, unblind, toy, localFiles, plot, save, years);
  else
    simfit_data_fullAngularMass_SwaveBin(q2Bin, parity, multiSample, nSample, q2stat, fitOption, XGBv, unblind, toy, localFiles, plot, save, years);
}
...
int main(int argc, char** argv)
{
  ...
  int toy=0;
  if ( argc > 9 ) toy = atoi(argv[9]);

  bool localFiles = false;
  if ( argc > 10 && atoi(argv[10]) > 0 ) localFiles = true;

  bool plot = true;
  int save = true;

  if ( argc > 11 && atoi(argv[11]) == 0 ) plot = false;
  if ( argc > 12 ) save = atoi(argv[12]);
...
//   cout << years[2] << endl;
  ...
  if ( q2Bin==-1 )
    for (q2Bin=0; q2Bin<nBins; ++q2Bin)
      simfit_data_fullAngularMass_SwaveBin1(q2Bin, parity, multiSample, nSample, q2stat, fitOption, XGBv, unblind, toy, localFiles, plot, save, years);
  else
    simfit_data_fullAngularMass_SwaveBin1(q2Bin, parity, multiSample, nSample, q2stat, fitOption, XGBv, unblind, toy, localFiles, plot, save, years);
